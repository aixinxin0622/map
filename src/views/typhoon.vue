...
export default {
  ...
  methods: {
    ...
    setupMap() {
      ...
      L.tileLayer('http://t5.tianditu.gov.cn/DataServer?T=ter_w&X={x}&Y={y}&L={z}&tk=189e6787797aa91b77083f384a289820', {}).addTo(this.map);
      this.map.on('zoomend', () => {
    ...
    typhoonTrack(data) {
      ...
      var intervalId = setInterval(function () {
        if (index < temp.length) {
          ...
          L.circleMarker(temp[index], { radius: 6, color: '#434343', fillColor: '#0062fe', fillOpacity: 1 }).addTo(self.TyphoonLineLayer)
            .bindTooltip(`
          ...
          tfyc.forEach((res) => {
            ...
            res.forecastpoints.forEach((item) => {
              ...
              L.circleMarker([item.lat, item.lng], { radius: 2, color: color, fillColor: color, fillOpacity: 1 }).addTo(self.typhoonArea);
            });
