...
export const usePermissionStore = defineStore('permission', () => {
  ...
  const filterAsyncRouter = (asyncRouterMap: RouteRecordRaw[], lastRouter?: RouteRecordRaw, type = false): RouteRecordRaw[] => {
    return asyncRouterMap.filter((route) => {
      ...
      const componentName = route.component?.toString();
      ...
      if (route.meta.link && route.meta.link.startsWith('http') && componentName === 'InnerLink') {
        // 设置到frameSrc  后续由vben处理
        route.meta.frameSrc = route.path;

        // route.path = route.path.replace('http://', '').replace('https://', '').replace(':', '/').replaceAll('.', '/');
        //
        // /**
        ...
        // }
        // 用 id 代替 path （ip 地址）
        route.path = route.path.replace(/http(s)?:\/\/.*/, route.meta.id as any);
      }
